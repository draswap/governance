...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IDraswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IDraswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = IDraswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IDraswapV2Pair(pair).transfer(pair, value));
                IDraswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IDraswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface IDraswapV2Pair {
